...
import stats.ConstantesColonnes.{COLSFORFAITFINALINTERNATIONAL, COLSFORFAITRECUINTERNATIONAL, COLSREGLEGESTIONFINAL}
...
object Forfaits {
  ...
  def procForfaits(
      spark: SparkSession,
      reglesGestionSrc: DataFrame,
      taxation: DataFrame,
      taxationInternational: DataFrame
  ): DataFrame = {

    logger.info("Debut procForfaitHorsForfait")

    // *********************************************************** Appel France *******************************************************//

    // Filtrer les appels par longueur du numéro appelant
    val reglesGestion = reglesGestionSrc
    ...
    val numerotationTaxation              = taxation
    val numerotationTaxationInternational = taxationInternational
    ...
    val reglesGestion08NumCourtUrgence =
      gestionCourtSpeciaux(reglesGestionLen8, "Forfait", "N° courts (urgence)", "", "")
    // *****************************************************************************************************************************************//
    ...
    val reglesGestionCourtService =
      gestionCourtSpeciaux(reglesGestionCourtServiceFiltered, "Forfait", "N° courts (services)", "", "")
    ...
    val reglesGestionCourtAutre =
      gestionCourtSpeciaux(reglesGestionCourtAutreFiltered, "Forfait", "N° courts (autre)", "", "")
    ...
    val internationalRecusSrc =
      reglesGestionInternational.where(f.col("Type_appel") === "Appel reçu").selectExpr(COLSFORFAITRECUINTERNATIONAL: _*)
