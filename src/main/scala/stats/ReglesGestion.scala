...
object ReglesGestion {

  // creation de la classe permettant de gérer les logs
  // val logger: Logger = Logger.getLogger(this.getClass.getName)

  def procReglesGestion(SviTaxationJoined: DataFrame): DataFrame = {
    // alimente forfaithrosforfait
    logger.info("Les regles de gestion")
    ...
    val sviTaxationSubstr = SviTaxationJoined
    ...
    val internCallsType4Grouped =
      internCallsType4.groupBy("InitialStartDateTime", "CalledNumber").agg(f.min("StartDateTime").as("NewStartDateTime"))
    ...
    unionCalls9to14 = unionCalls9to14
      ...
      .withColumn(
        "Aboutissement_SVI",
        f.when(f.col("endcause") === "RELEASED By VAA", "Le SVI raccroche")
          .when(
            f.col("endcause") === "RELEASED By Caller" && f.col("Motif_appel").isNull && f.col("bloc_duration") <= 60,
          ...
          .when(f.col("endcause") === "RELEASED By Caller" && f.col("Motif_appel").isNotNull, "Mise en relation")
          .when(f.col("endcause") === "TRANSFERRED", "Mise en relation")
          .otherwise(null)
      )
    ...
    val reglesFinal = unionCalls9to14
      ...
      .withColumn(
        ...
        f.when(f.col("endcause") === "TRANSFERRED", f.col("Appels_recus"))
          .when(
            f.col("endcause") === "RELEASED By Caller" && f.col("Motif_appel").isNotNull && f.col("bloc_duration").cast(
              "float"
            ) > 60,
            f.col("Appels_recus")
          )
      ...
      .withColumn("Appels_dissuades", f.when(f.col("endcause") === "RELEASED By VAA", f.col("Appels_recus")))
      .withColumn(
        "Appels_raccroches",
        f.when(
          f.col("endcause") === "RELEASED By Caller" && (f.col("Motif_appel").isNull || (f.col(
            "Motif_appel"
      ...
      .withColumn("Appels_SVI_non_reconnu", f.when(f.col("CallType") === 9 && f.col("endcause").isNull, f.col("Appels_recus")))
      .withColumn(
        "Appels_directs_internes",
        f.when(f.col("Origine") === "Direct" && f.col("calledNumber_externe_interne") === "Interne", f.col("Appels_recus"))
      )
      .withColumn(
        "Appels_directs_externes",
        f.when(f.col("Origine") === "Direct" && f.col("calledNumber_externe_interne") === "Externe", f.col("Appels_recus"))
      )
      .withColumn(
        "Appels_aboutis",
        f.when(
          f.col("Appels_directs_internes") === 1 || f.col("Appels_directs_externes") === 1 || f.col("Mise_en_relation") === 1,
          f.col("Appels_recus")
        )
      )
      ...
      .withColumn(
        ...
        f.when(
          (f.col("Appels_aboutis") === 1 && f.col("EffectiveCallDuration") === 0) || (f.col(
            "endcause"
          ) === "RELEASED By Caller" && f.col("Motif_appel").isNotNull && f.col("bloc_duration") > 60),
          f.col("Appels_recus")
        )
      )
      .withColumn("Appels_transferes", f.when(f.col("Appel_transfere") === "1.0", f.col("Appel_transfere").cast("Integer")))
      .withColumn(
        "Appels_renvoyes",
        f.when(
    ...
    logger.info("Fin de la session RG")
    reglesDeGestion
