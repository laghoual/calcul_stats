...
object SviPrep extends SparkModel {

  override val declaredVariables: Set[String] = Set()

  override val inputs: Set[SparkModel] = Set(SviNodeSource, SviStatSource)

  override val materialization: Materialization = Materialization.LOCAL_CHECKPOINT
  ...
  override protected def compute(implicit variables: Variables): DataFrame = {
    ...
    if (!controleColonnesDf(SviNodeSource, listColumns)) {
    ...
    val df_sviNode = SviNodeSource
      ...
      .withColumn(
        ...
        f.concat_ws(
          ...
          f.substring(f.col("timestamp"), -3, 3)
        )
      )
    ...
    if (!controleColonnesDf(SviStatSource, listColumnsSvi)) {
    ...
    val sviStatFormated = SviStatSource
      .selectExpr(
        "id",
        "caller",
        "called",
