...
object SviTaxJoinFloue extends SparkModel {

  override val declaredVariables: Set[String] = Set()

  override val inputs: Set[SparkModel] = Set(SviPrep, TaxationPrep)
  ...
  override protected def compute(implicit variables: Variables): DataFrame = {
    ...
    val taxationSelected = TaxationPrep.selectExpr(
    ...
    val sviSelected = SviPrep.selectExpr(
      "started_datetime",
      "temp_caller",
      "endcause"
    )
