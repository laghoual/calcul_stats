...
object Forfaits2 extends SparkModel {
  ...
  def gestionCourtSpeciaux(
    ...
    //var df: DataFrame = reglesGestionLen8
    reglesGestionLen8
      .withColumn("Appel_facturable", f.lit(appel_facturable))
      .withColumn(
        "Appel_vers",
        f.lit(Appel_vers)
        // f.when(f.lit(Appel_vers) === "N° courts (autre)", f.lit("Numéros internes")).otherwise(f.lit("Numéros d'urgence"))
        // if f.lit(Appel_vers) === "N° courts (autre)"{f.lit("Numéros internes") }
      )
    ...
    //df.withColumn(
      ...
      //f.when(f.col("Appel_vers") === "N° courts (services)", f.lit("Numéros d'urgence"))
        //.when(
          //(f.col("Appel_vers") === "N° courts (autre)")
            //|| (f.col("Appel_vers") === "N° courts (urgence)"),
          //f.lit("Numéros interne")
        //)
    //)

    // .when(f.col("Appel_vers") === "N° courts (urgence)", f.lit("Numéros interne")))
    // .otherwise(f.when(f.col("Appel_vers") === "N° courts (autre)", f.lit("Numéros interne"))
    // .otherwise(f.when(f.col("Appel_vers") === "N° courts (urgence)", f.lit("Numéros interne")))))
  }
}
